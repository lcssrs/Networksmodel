...
df2 <- df[,c("FE_VIA", "MODOPRIN", "DURACAO", "ZONA_O", "ZONA_D", "H_SAIDA", "DISTANCIA", "ZONA_T1", "ZONA_T2", "ZONA_T3", "MOTIVO_O", "MOTIVO_D" )]

#Selecting motorized travels and no rush hours
df1 <- subset(df2, (df2$MODOPRIN==12 | df2$MODOPRIN==13)  )

#& (between(df2$MODOPRIN,9,13) | between(df2$MODOPRIN,1,3))
#df1 <- subset(df2, df2$MODOPRIN<15)

df1$VELOCIDADE <- df1$DISTANCIA/df1$DURACAO
df1<- df1[,c("FE_VIA", "VELOCIDADE", "ZONA_O", "ZONA_D", "DURACAO", "ZONA_T1", "ZONA_T2", "ZONA_T3")]
...
for (i1 in 1:nrow(matrix1)){
  for (j1 in 1:ncol(matrix1)){
    ...
    if (length(A1$FE_VIA)>0){  
      for (k in length(A1$FE_VIA)) {
        
        if (is.na(A1$ZONA_T1[k])){
          matrix1[i1,j1] <- matrix1[i1,j1] + A1$FE_VIA[k]*A1$VELOCIDADE[k]
        } else {
          a1<-A1$ZONA_T1[k]
          matrix1[i1,a1] <- matrix1[i1,a1] + A1$FE_VIA[k]*A1$VELOCIDADE[k]
          
          if (is.na(A1$ZONA_T2[k])){
            matrix1[a1,j1] <- matrix1[a1,j1] + A1$FE_VIA[k]*A1$VELOCIDADE[k]
          } else {
            a2<-A1$ZONA_T2[k]
            matrix1[a1,a2] <- matrix1[a1,a2] + A1$FE_VIA[k]*A1$VELOCIDADE[k]
            
            if (is.na(A1$ZONA_T3[k])){
              matrix1[a2,j1] <- matrix1[a2,j1] + A1$FE_VIA[k]*A1$VELOCIDADE[k]
            } else{ 
              a3<-A1$ZONA_T3[k]
              matrix1[a2,a3] <- matrix1[a2,a3] + A1$FE_VIA[k]*A1$VELOCIDADE[k]
              matrix1[a3,j1] <- matrix1[a3,j1] + A1$FE_VIA[k]*A1$VELOCIDADE[k]
            }
          }
        }  
      }
...
#matrix1[row(matrix1)==col(matrix1)]=0
...
df3 <- subset(df2, (df2$MODOPRIN==12 | df2$MODOPRIN==13) & (between(df2$H_SAIDA,7,10) | between(df2$H_SAIDA,17,20)) )
...
#matrix3[row(matrix3)==col(matrix3)]=0
...
for (h in 1:517){
  ...
  correctclo[h,]<-c(clocentd[correspondent,1],h)
  
  correctout[h,]<-c(clocentout[correspondent,1],h)
  
  correcteig[h,]<-c(eigcentd[correspondent,1],h)
  
  correcthub[h,]<-c(hubscoro[correspondent,1],h)
  
  correctaut[h,]<-c(autscord[correspondent,1],h)
}
...
write.dta(correctaut, "Banco de Dados\\07_autscord_adj.dta")
